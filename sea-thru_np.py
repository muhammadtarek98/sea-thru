...
def construct_neighborhood_map(depths, epsilon=0.05):
    ...
    if len(zeros_size_arr) > 0:
        nmap[nmap == zeros_size_arr[0][0]] = 0  # reset the largest background to 0
...
def run_pipeline(img, depths, min_depth, p, f, l, spread_data_fraction):
    ...
    refined_beta_D_r, coefsR = refine_wideband_attentuation(depths, illR, beta_D_r,
                                                            radius_fraction=spread_data_fraction, l=l)
...
if __name__ == '__main__':
    ...
    preprocess_for_monodepth = True  # Preprocess for monodepth depth maps
    monodepth = False  # Preprocess for monodepth
    equalize_image = True  # Histogram equalization for final output
    img_folder = '/home/cplus/projects/m.tarek_master/Image_enhancement/depth_maps/images'
    dep_folder = '/home/cplus/projects/m.tarek_master/Image_enhancement/depth_maps/depth_images'
    result = '/home/cplus/projects/m.tarek_master/Image_enhancement/sea-thru/result'
    os.makedirs(result, exist_ok=True)
    images = sorted(os.listdir(img_folder))
    depth_maps = sorted(os.listdir(dep_folder))
    for i in range(len(depth_maps)):
        image_name = os.path.join(img_folder, images[i])
        depth_map_name = os.path.join(dep_folder, depth_maps[i])
        image, depth_map = load_image_and_depth_map(image_name, depth_map_name)
        recovered_image = run_pipeline(img=image, depths=depth_map, p=p, f=f, l=l,
                                       spread_data_fraction=spread_data_fraction, min_depth=min_depth)
        plt.imsave(os.path.join(result, image_name), np.clip(recovered_image, a_min=0, a_max=1))
    """for i in range(len(data)):
        rgb = os.path.join(img_folder, images[i])
        dep = os.path.join(dep_folder, depth_maps[i])
        print(rgb)
        print(dep)

        #if preprocess_for_monodepth:
            # generate depth map
            #preprocess_for_monodepth(rgb, os.path.join(result, images[i]))
        ...
        plt.imsave(os.path.join(result, rgb), np.clip(recovered, a_min=0, a_max=1))
"""
